    ...
    if (!verify_csrf($_POST['csrf_token'] ?? '')) {
        ...
        if (!$errors) {
            ...
            if ($existing) {
                ...
                $ok = db_execute(
                    'INSERT INTO users (first_name, last_name, email, password_hash, role, country, phone) VALUES (?,?,?,?,?,?,?)',
                    [$first_name, $last_name, $email, $hash, $role, $country, $phone]
                );
                // Even if no DB is connected yet, let the prototype flow continue.
                $success = true;
            }
        }
    }
