...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf($_POST['csrf_token'] ?? '')) {
        ...
        // Demo auth (replace with PDO query + password_verify)
        if ($email === 'player@demo.com' && $password === 'demo') {
            $_SESSION['user_id'] = 1;
            $_SESSION['user']    = ['id' => 1, 'first_name' => 'Alex', 'last_name' => 'Demo', 'role' => 'player', 'email' => $email];
            redirect('dashboard.php');
        } elseif ($email === 'instructor@demo.com' && $password === 'demo') {
            $_SESSION['user_id'] = 2;
            $_SESSION['user']    = ['id' => 2, 'first_name' => 'Dr. Jordan', 'last_name' => 'Smith', 'role' => 'instructor', 'email' => $email];
            redirect('dashboard.php');
        } else {
