...
$rows = db_fetch_all(
    'SELECT g.id, s.name, g.current_round, s.total_rounds, g.score, g.status, g.deadline
...
foreach ($rows as $row) {
    $active_sims[] = [
        'id' => $row['id'], 'name' => $row['name'], 'round' => (int)$row['current_round'],
        'total_rounds' => (int)$row['total_rounds'], 'score' => (float)$row['score'],
        'rank' => null, 'status' => $row['status'], 'deadline' => $row['deadline'],
    ];
}

$rows = db_fetch_all(
    'SELECT s.name AS sim, r.round, r.revenue, r.profit, r.rank, r.created_at
     FROM results r JOIN games g ON g.id = r.game_id JOIN simulations s ON s.id = g.simulation_id
     WHERE g.user_id = ? ORDER BY r.created_at DESC LIMIT 5',
    [$user['id']]
);
foreach ($rows as $row) {
    $recent_results[] = [
        'sim' => $row['sim'], 'round' => $row['round'], 'revenue' => (float)$row['revenue'],
        'profit' => (float)$row['profit'], 'rank' => $row['rank'], 'date' => substr($row['created_at'], 0, 10),
    ];
}

// Demo fallback data if DB has nothing yet (fresh install / no MySQL configured)
if (!$active_sims) {
    $active_sims = [
...
$active_count = count(array_filter($active_sims, fn($s) => $s['status'] === 'active'));
$best_rank_sim = null;
$best_rank = null;
foreach ($active_sims as $s) {
    if ($s['rank'] !== null && ($best_rank === null || $s['rank'] < $best_rank)) {
        $best_rank = $s['rank'];
        $best_rank_sim = $s['name'];
    }
}
$avg_score = $active_sims ? round(array_sum(array_column($active_sims, 'score')) / count($active_sims)) : 0;
...
<div class="stats-grid">
  <div class="stat-card blue">
    ...
    <div class="stat-card-delta up"><i class="fa fa-arrow-up"></i> Keep it up</div>
  </div>
  <div class="stat-card green">
    <div class="stat-card-icon"><i class="fa fa-trophy"></i></div>
    <div class="stat-card-label">Best Rank</div>
    <div class="stat-card-value"><?= $best_rank ? '#' . $best_rank : '—' ?></div>
    <div class="stat-card-delta up"><i class="fa fa-arrow-up"></i> <?= e($best_rank_sim ?? 'No ranked games yet') ?></div>
  </div>
  <div class="stat-card orange">
    ...
    <div class="stat-card-delta up"><i class="fa fa-arrow-up"></i> Across all simulations</div>
  </div>
  <div class="stat-card red">
    <div class="stat-card-icon"><i class="fa fa-clock"></i></div>
    <div class="stat-card-label">Decisions Due</div>
    <div class="stat-card-value"><?= $active_count ?></div>
    <div class="stat-card-delta down"><i class="fa fa-circle-exclamation"></i> Check deadlines below</div>
  </div>
</div>
      ...
      <div style="padding: 16px 0; border-bottom: 1px solid var(--border);">
        <div class="d-flex align-center justify-between mb-8">
          ...
          <div class="d-flex align-center gap-12">
            ...
            <span class="text-sm fw-600">Rank <?= $sim['rank'] !== null ? '#' . $sim['rank'] : '—' ?></span>
